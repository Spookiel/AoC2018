data = """position=< -9767,  50146> velocity=< 1, -5>
...
def run(data):
    ...
    for i in range(20000):
        minx = min(x + i * vx for (x, y, vx, vy) in info)
        maxx = max(x + i * vx for (x, y, vx, vy) in info)
        miny = min(y + (i * vy) for (x, y, vx, vy) in info)
        maxy = max(y + (i * vy) for (x, y, vx, vy) in info)
        small[i] = maxx - minx + maxy - miny

    for k,v in small.items():
        if v == min(small.values()):
            smallest = k
            break"""
    ans = [[' '] * 250 for j in range(200)]
    i = 10003
    ma, mb = [], []
    for (x, y, vx, vy) in info:
        a, b = y + i * vy,x + i * vx
        ma.append(a)
        mb.append(b)
        try:
