data = """[1518-03-11 00:45] wakes up
...
def run(data):
    ...
    guard_mins_asleep = defaultdict(int)
    ...
    for line in lines:
        ...
        if "begins" in line:
            ...
            for t in range(awake, time):
                guard_mins_asleep[guard] += 1
                times_asleep[(guard, t)] += 1
    return guard_mins_asleep, times_asleep


g_m_a, t_a = run(data)


def get_ans_part1(gma, ta):
    ans = None
    for i, j in gma.items():
        if j == max(gma.values()):
            ans = i
    best = None
    best_min = None
    ...
    print(best_min * ans, "part 1")


def get_ans_part2(gma, ta):
    best = None
    best_min = None
    best_guard = None
    for k, v in ta.items():
    ...
    print(best_min * best_guard, "part 2")


get_ans_part2(g_m_a, t_a)
